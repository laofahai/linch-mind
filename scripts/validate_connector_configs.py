...
class ConnectorConfigValidator:
    ...
    def validate_all(self) -> bool:
        ...
        try:
            ...
            # 6. 验证文件系统一致性
            self._validate_filesystem_consistency(master_config.get("connector_types", {}))
    ...
    def _validate_single_connector_type(self, type_id: str, type_config: Dict):
        ...
        if not entry_point.endswith(".py"):
            self.warnings.append(f"连接器类型 {type_id} 入口点不是Python文件: {entry_point}")
    ...
    def _validate_connector_instances(self, instances: Dict[str, Dict], connector_types: Dict[str, Dict]):
        """验证连接器实例配置"""
        ...
        for instance_id, instance_config in instances.items():
            self._validate_single_instance(instance_id, instance_config, connector_types)
    ...
    def _validate_single_instance(self, instance_id: str, instance_config: Dict, connector_types: Dict):
        """验证单个连接器实例"""
        # 必需字段检查
        ...
        if type_id and type_id not in connector_types:
            self.errors.append(f"连接器实例 {instance_id} 引用了不存在的类型: {type_id}")
            return
        
        # 状态值检查
        state = instance_config.get("state", "configured")
        valid_states = ["available", "installed", "configured", "enabled", "running", "error", "stopping", "updating", "uninstalling"]
        ...
        if type_id and type_id in connector_types:
            ...
            if config_schema and not self._validate_instance_config(instance_config_data, config_schema):
                self.warnings.append(f"连接器实例 {instance_id} 配置可能不符合类型要求")
    
    def _validate_instance_config(self, instance_config: Dict, config_schema: Dict) -> bool:
        """验证实例配置是否符合类型要求（简化版）"""
    ...
    def _check_instance_limits(self, instances: Dict, connector_types: Dict):
        ...
        for type_id, count in type_instance_counts.items():
            if type_id in connector_types:
                connector_type = connector_types[type_id]
                supports_multiple = connector_type.get("supports_multiple_instances", False)
                max_instances = connector_type.get("max_instances_per_user", 1)
                
                if not supports_multiple and count > 1:
                    self.errors.append(f"连接器类型 {type_id} 不支持多实例，但配置了 {count} 个实例")
                
                if count > max_instances:
                    self.errors.append(f"连接器类型 {type_id} 实例数量 ({count}) 超过最大限制 ({max_instances})")
    ...
    def _validate_filesystem_consistency(self, connector_types: Dict[str, Dict]):
        ...
        for type_id, type_config in connector_types.items():
            ...
            if not connector_dir.exists():
                self.warnings.append(f"连接器类型 {type_id} 的目录不存在: {connector_dir}")
                continue
            ...
            if not entry_file.exists():
                self.errors.append(f"连接器类型 {type_id} 的入口文件不存在: {entry_file}")
            ...
            if connector_json.exists():
                try:
                    ...
                    if local_id and local_id != type_id:
                        self.warnings.append(f"连接器 {type_id} 本地配置ID不匹配: {local_id}")
        ...
        for connector_dir in official_dir.iterdir():
            if connector_dir.is_dir() and connector_dir.name not in connector_types:
                ...
                if main_file.exists():
                    self.warnings.append(f"文件系统中存在未配置的连接器: {connector_dir.name}")
        
        self.info.append("✅ 文件系统一致性验证完成")
    
    def _validate_config_completeness(self, master_config: Dict, instances_config: Dict):
        """验证配置完整性"""
        logger.info("🔧 验证配置完整性...")
        ...
        if types_without_instances:
            self.info.append(f"以下连接器类型没有配置实例: {', '.join(types_without_instances)}")
    ...
    def _print_validation_results(self):
        ...
        print(f"总计问题: {total_issues} (错误: {len(self.errors)}, 警告: {len(self.warnings)})")
