  ...
  linch dev start
    ...
    def _add_connector_commands(self, subparsers):
        ...
        list_parser.add_argument(
            "--running", action="store_true", help="只显示运行中的连接器"
        )
        ...
        logs_parser.add_argument(
            "--follow", "-f", action="store_true", help="实时跟踪日志"
        )
    ...
    def _add_daemon_commands(self, subparsers):
        ...
        start_parser.add_argument(
            "--port", type=int, default=get_daemon_port(), help="端口号"
        )
        start_parser.add_argument(
            "--background", "-d", action="store_true", help="后台运行"
        )
    ...
    def _add_config_commands(self, subparsers):
        ...
        init_parser.add_argument(
            "--force", action="store_true", help="强制重写现有配置"
        )
    ...
    async def cmd_connector_create(self, args):
        ...
        config = {
            ...
            "category": "general",
            "entry": {
                "development": {"args": ["main.py"], "working_dir": "."},
                "production": {
...
'''
    ...
    async def cmd_dev_start(self, args):
        ...
        # 如果指定了连接器，启动它
        if args.connector:
            await self.cmd_connector_start(
                type("Args", (), {"connector_id": args.connector, "config": None})()
            )
    ...
    async def cmd_dev_test(self, args):
        ...
        if args.connector:
            ...
            test_cmd = [
                ...
                "run",
                "pytest",
                f"tests/test_{args.connector}.py",
                "-v",
            ]
        else:
            # 运行所有测试
    ...
    def _print_connector_table(
        self, connectors: List[dict], running_only: bool = False
    ):
        """打印连接器表格"""
        if not connectors:
            print("📭 没有找到连接器")
        ...
        for connector in connectors:
            ...
            print(
                f"{connector['id']:<15} {connector['name']:<20} {status_icon}{status:<14} {data_count:<10}"
            )
