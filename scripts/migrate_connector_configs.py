...
class ConnectorConfigMigrator:
            ...
            for connector_data in connectors:
                try:
                    ...
                    type_data = {
                        ...
                        "display_name": connector_info.get(
                            "display_name", connector_info.get("name", "")
                        ),
                        ...
                        # 能力配置
                        "supports_multiple_instances": capabilities.get(
                            "supports_multiple_instances", False
                        ),
                        "max_instances_per_user": capabilities.get(
                            "max_instances_per_user", 1
                        ),
                        "auto_discovery": capabilities.get("auto_discovery", False),
                        "hot_config_reload": capabilities.get(
                            "hot_config_reload", True
                        ),
                        "health_check": capabilities.get("health_check", True),
                        # 技术信息
                        "entry_point": connector_info.get("entry_point", "main.py"),
                        "python_version": system_requirements.get(
                            "python_version", ">=3.8"
                        ),
                        "dependencies": connector_data.get("package_info", {}).get(
                            "dependencies", []
                        ),
        ...
        for connector_dir in self.official_dir.iterdir():
            ...
            try:
                ...
                type_data = {
                    ...
                    "display_name": connector_config.get(
                        "display_name", connector_config.get("name", "")
                    ),
                    ...
                    # 从capabilities部分提取
                    "supports_multiple_instances": connector_config.get(
                        "capabilities", {}
                    ).get("supports_multiple_instances", False),
                    "max_instances_per_user": connector_config.get(
                        "capabilities", {}
                    ).get("max_instances_per_user", 1),
                    # 技术信息
                    ...
                    "instance_templates": connector_config.get(
                        "instance_templates", []
                    ),
                }
    ...
    def _cleanup_old_files(self):
        ...
        if self.old_registry_path.exists():
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_registry = (
                self.old_registry_path.parent / f"registry_backup_{timestamp}.json"
            )
    ...
    def _print_migration_summary(self, connector_types: Dict, instances: Dict):
        ...
        for type_id, type_data in connector_types.items():
            print(
                f"  - {type_id}: {type_data.get('name', 'Unknown')} v{type_data.get('version', '1.0.0')}"
            )
