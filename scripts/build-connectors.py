...
class ConnectorBuilder:
    ...
    def build_connector(self, connector: Dict, target_platform: str = None) -> bool:
        ...
        try:
            ...
            pyinstaller_args = [
                ...
                "--onefile",
                "--name",
                output_name.replace(executable_suffix, ""),
                "--distpath",
                str(connector_path),
                "--workpath",
                str(build_work_dir),
    ...
    def package_connector(
        self, connector: Dict, target_platform: str = None
    ) -> Optional[Path]:
        """打包连接器"""
        if target_platform is None:
            target_platform = PlatformInfo.get_current_platform()
    ...
    def _create_install_script(
        self, package_dir: Path, connector: Dict, target_platform: str
    ):
        """创建安装脚本"""
        connector_id = connector["id"]
...
"""
    ...
    def build_all(
        self, target_platforms: List[str] = None
...
class ConnectorPackageManager:
    ...
    def list_packages(self) -> List[Dict]:
        ...
        return sorted(
            packages, key=lambda x: (x["connector_id"], x["version"], x["platform"])
        )
    ...
    def print_packages(self):
        ...
        for package in packages:
            ...
            print(
                f"   {package['version']}-{package['platform']}: {size_mb:.2f} MB ({package['modified_time'][:19]})"
            )
...
def main():
    ...
    package_parser.add_argument(
        "connector", nargs="?", help="Specific connector to package"
    )
    package_parser.add_argument(
        "--platform",
        choices=PlatformInfo.get_supported_platforms(),
    ...
    try:
        ...
        if args.command == "build":
            ...
            target_connector = next(
                (c for c in connectors if c["id"] == args.connector), None
            )
            ...
            successful_builds = sum(
                ...
                for success in platform_results.values()
                if success is True
            )
            ...
            if args.connector:
                ...
                package_path = builder.package_connector(
                    target_connector, args.platform
                )
