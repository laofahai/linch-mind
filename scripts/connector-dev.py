...
class ConnectorValidator:
    ...
    def validate_connector(self, connector_dir: Path) -> bool:
        ...
        try:
            ...
            if missing_fields:
                logger.error(f"Missing required fields in connector.json: {missing_fields}")
                return False
            
            # 验证ID匹配目录名
            if config["id"] != connector_dir.name:
                logger.error(f"Connector ID '{config['id']}' doesn't match directory name '{connector_dir.name}'")
                return False
    ...
    def validate_all(self) -> bool:
        ...
        connectors = [d for d in OFFICIAL_DIR.iterdir() if d.is_dir() and not d.name.startswith('.')]
        ...
        logger.info(f"Validation complete: {valid_count}/{len(connectors)} connectors valid")
        ...
        for connector in connectors:
            ...
            if detail:
                ...
                if connector['supported_platforms']:
                    print(f"   Platforms: {', '.join(connector['supported_platforms'])}")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="Linch Mind Connector Development Tools")
    subparsers = parser.add_subparsers(dest="command", help="Available commands")
    
    # validate命令
    validate_parser = subparsers.add_parser("validate", help="Validate connectors")
    validate_parser.add_argument("connector", nargs="?", help="Specific connector to validate (optional)")
