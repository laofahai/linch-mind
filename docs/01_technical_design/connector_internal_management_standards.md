...
### 简化的职责分离
- **Daemon职责**: 连接器生命周期管理、配置分发、状态监控
- **连接器职责**: 数据采集、处理、推送到Daemon API

### 核心设计原则
1. **简单优先**: 避免过度工程化，专注核心功能
2. **Python生态**: 充分利用Python丰富的第三方库
3. **Poetry管理**: 统一的依赖管理和环境隔离
4. **API驱动**: 通过Daemon API推送数据，而非复杂IPC
