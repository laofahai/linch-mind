...
async def register_custom_widget(
    ...
    try:
        ...
        if not connector_id or not widget_name:
            raise HTTPException(
                status_code=400, 
                detail="connector_id and widget_name are required"
            )
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to register custom widget"
            )
...
async def serve_static_file(
    ...
    try:
        ...
        content_type = mimetypes.guess_type(str(file_full_path))[0] or "application/octet-stream"
