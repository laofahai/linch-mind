...
async def get_connector_config_schema(
    ...
    try:
        ...
        if not schema:
            raise HTTPException(
                status_code=404, detail=f"连接器 {connector_id} 不存在或无配置schema"
            )
...
async def validate_config(
    ...
    try:
        validation_result = await config_service.validate_config(
            request.connector_id, request.config
        )
        ...
        return ApiResponse(
            ...
            data={
                ...
                "normalized_config": validation_result.get(
...
async def reset_config(
    ...
    try:
        result = await config_service.reset_config(
            connector_id=request.connector_id, to_defaults=request.to_defaults
        )
...
async def get_all_connector_schemas(
    ...
    try:
        ...
        return ApiResponse(
            success=True,
            data={"schemas": schemas, "total_count": len(schemas)},
...
async def register_connector_schema(
    ...
    try:
        ...
        # 注册运行时schema
        success = await config_service.register_runtime_schema(
            connector_id, schema_data
        )
