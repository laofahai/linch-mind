...
async def validate_config(request: IPCRequest) -> IPCResponse:
    ...
    if not connector_id or config is None:
        return IPCResponse(
            status_code=400, data={"error": "Missing connector_id or config"}
        )
...
async def update_config(request: IPCRequest) -> IPCResponse:
    ...
    try:
        ...
        if not validation_result["valid"]:
            return IPCResponse(
                success=False, data=validation_result, message="配置验证失败"
            )

        # 更新配置
        success = await service.update_config(connector_id, config, change_reason)
        ...
        if success:
            ...
            return IPCResponse(success=False, message=f"更新 {connector_id} 的配置失败")
