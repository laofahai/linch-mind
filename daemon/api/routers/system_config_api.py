...
async def get_registry_config():
    ...
    try:
        ...
        return RegistryConfigResponse(
            ...
            current_source=(
                status["last_successful_source"]["description"]
                if status["last_successful_source"]
                else None
            ),
...
async def update_registry_config(config_request: RegistryConfigRequest):
    ...
    try:
        ...
        if config_request.registry_url is not None:
            core_config.config.connector_registry.url = config_request.registry_url

        if config_request.cache_duration_hours is not None:
            core_config.config.connector_registry.cache_duration_hours = (
                config_request.cache_duration_hours
            )

        if config_request.auto_refresh is not None:
            core_config.config.connector_registry.auto_refresh = (
                config_request.auto_refresh
            )
...
async def test_registry_url(test_url: str):
    ...
    try:
        ...
        if registry_data and discovery_service._validate_registry_data(registry_data):
            ...
            return {"status": "error", "message": "URL不可用或数据格式错误"}
...
async def refresh_registry():
    ...
    try:
        ...
        if registry_data:
            ...
            return {"status": "error", "message": "刷新注册表失败"}
...
async def get_registry_connectors(
...
async def get_connector_download_info(connector_id: str, platform: str = "linux-x64"):
    ...
    try:
        ...
        if download_info:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"连接器 {connector_id} 不存在或不支持平台 {platform}",
            )
