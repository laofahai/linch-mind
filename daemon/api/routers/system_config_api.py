...
async def get_registry_config(request: IPCRequest) -> IPCResponse:
    ...
    try:
        ...
        response_data = {
            ...
            "current_source": status.get("last_successful_source", {}).get("description"),
...
async def update_registry_config(request: IPCRequest) -> IPCResponse:
    ...
    try:
        ...
        if "cache_duration_hours" in body:
            core_config.config.connector_registry.cache_duration_hours = body["cache_duration_hours"]
...
async def test_registry_url(request: IPCRequest) -> IPCResponse:
    ...
    try:
        ...
        discovery_service = get_registry_discovery_service()
        test_source = RegistrySource(type=RegistrySourceType.USER_CONFIGURED, url=test_url, priority=1, timeout=30, description="测试URL")
        ...
        if registry_data and discovery_service._validate_registry_data(registry_data):
            ...
            return IPCResponse.success_response(data={
                "status": "success", "message": f"URL可用，发现 {count} 个连接器",
                "connector_count": count, "last_updated": registry_data.get("last_updated")
            })
        else:
            return IPCResponse.success_response(data={"status": "error", "message": "URL不可用或数据格式错误"})

    except Exception as e:
        logger.error(f"测试注册表URL失败: {e}")
        return IPCResponse.success_response(data={"status": "error", "message": f"测试失败: {str(e)}"})
...
async def refresh_registry(request: IPCRequest) -> IPCResponse:
    ...
    try:
        ...
        if registry_data:
            count = len(registry_data.get("connectors", {}))
            return IPCResponse.success_response(data={
                "status": "success", "message": f"注册表已刷新，发现 {count} 个连接器",
                "source": source.description if source else "unknown", "connector_count": count
            })
        else:
            return IPCResponse.success_response(data={"status": "error", "message": "刷新注册表失败"})

    except Exception as e:
        logger.error(f"刷新注册表失败: {e}")
        return IPCResponse.success_response(data={"status": "error", "message": f"刷新失败: {str(e)}"})
