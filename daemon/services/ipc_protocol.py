...
class IPCResponse:
    ...
    def error_response(
        ...
        """创建错误响应"""
        error_code_str = error_code.value if isinstance(error_code, IPCErrorCode) else error_code
...
def connection_failed_response(message: str, details: Optional[Dict[str, Any]] = None) -> IPCResponse:
    """连接失败响应"""
...
def invalid_request_response(message: str, details: Optional[Dict[str, Any]] = None) -> IPCResponse:
    """无效请求响应"""
    return IPCResponse.error_response(
...
def internal_error_response(message: str, details: Optional[Dict[str, Any]] = None) -> IPCResponse:
    """内部错误响应"""
    return IPCResponse.error_response(
