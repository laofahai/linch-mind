...
from ..ipc_protocol import (
    ...
    invalid_request_response, internal_error_response, resource_not_found_response
)
...
def create_webview_config_router() -> IPCRouter:
    ...
    async def check_webview_support(request: IPCRequest) -> IPCResponse:
        ...
        try:
            from daemon.services.connectors.connector_config_service import get_connector_config_service
            
            service = get_connector_config_service()
            schema_data = await service.get_config_schema(connector_id)
            
            supports_webview = False
            if schema_data and schema_data.get("source", "").startswith("static_manifest"):
    ...
    async def get_webview_config_html(request: IPCRequest) -> IPCResponse:
        ...
        try:
            ...
            current_config = current_config_data.get("current_config", {}) if current_config_data else {}
    ...
    async def save_custom_template(request: IPCRequest) -> IPCResponse:
        ...
        if not template_name:
            return invalid_request_response(
                "Missing template_name in path",
                {"required_param": "template_name"}
            )
