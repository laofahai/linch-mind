...
class WindowsIPCServer:
    ...
    def __init__(self, pipe_name: Optional[str] = None):
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=10, thread_name_prefix="NamedPipe"
        )
    ...
    def _setup_security(self):
        ...
        try:
            ...
            logger.info(
                f"Windows Named Pipe安全设置完成: 仅允许用户 {current_user} 访问"
            )
        ...
        try:
            ...
            if self.app:
                ...
                response = IPCResponse(
                    status_code=503, data={"error": "IPC application not available"}
                )
            ...
            error_response = IPCResponse(
                status_code=500, data={"error": f"Internal server error: {str(e)}"}
            )
