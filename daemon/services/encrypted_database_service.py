    ...
    def _setup_sqlcipher_pragmas(self):
        ...
        def set_sqlcipher_pragmas(dbapi_connection, connection_record):
            ...
            try:
                ...
                cursor.execute("PRAGMA kdf_iter = 256000;")             # PBKDF2迭代次数 (OWASP推荐)
                cursor.execute("PRAGMA cipher_page_size = 4096;")       # 4KB页面大小(性能平衡)
                cursor.execute("PRAGMA cipher_use_hmac = ON;")          # 启用HMAC完整性验证
                cursor.execute("PRAGMA cipher_plaintext_header_size = 32;") # 明文头部大小
...
def create_encrypted_database_service(encryption_key: Optional[str] = None) -> EncryptedDatabaseService:
    """创建加密数据库服务实例"""
