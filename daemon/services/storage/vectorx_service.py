...
try:
    ...
    logger.warning(
        "VectorX Python SDK not available. Install with: pip install vectorx-python"
    )
...
class VectorXService:
    ...
    def __init__(
        ...
        if not VECTORX_AVAILABLE:
            raise ImportError(
                "VectorX Python SDK is required. Install with: pip install vectorx-python"
            )
        ...
        self.index_name = index_name or os.getenv(
            "VECTORX_INDEX_NAME", "linch_mind_vectors"
        )
    ...
    async def add_document(self, document: VectorXDocument) -> bool:
        ...
        try:
            ...
            result = await self.vx.upsert(
                index_name=self.index_name, vectors=vectors, key=self.encryption_key
            )
    ...
    async def get_document(self, document_id: str) -> Optional[VectorXDocument]:
        ...
        try:
            ...
            result = await self.vx.fetch(
                index_name=self.index_name, ids=[document_id], key=self.encryption_key
            )
            ...
            if result.get("vectors") and len(result["vectors"]) > 0:
                ...
                return VectorXDocument(
                    ...
                    created_at=(
                        datetime.fromisoformat(doc_data["metadata"]["created_at"])
                        if doc_data["metadata"].get("created_at")
                        else None
                    ),
                )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            stats = await self.vx.describe_index_stats(
                index_name=self.index_name, key=self.encryption_key
            )
            ...
            return {"error": str(e), "total_documents": self.total_documents}
