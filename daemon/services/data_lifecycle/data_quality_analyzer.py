...
class DataQualityAnalyzer:
    ...
    def analyze_data_quality(self) -> DataQualityReport:
        ...
        try:
            with self.db_service.get_session() as session:
                # 基础统计
                total_entities = session.scalar(
                    select(func.count()).select_from(EntityMetadata)
                ) or 0
                ...
                # 描述完整性分析
                description_coverage = self._analyze_description_coverage(session, total_entities)
                
                # 关系完整性分析
                relationship_coverage = self._analyze_relationship_coverage(session, total_entities)
    ...
    def _analyze_description_coverage(self, session, total_entities: int) -> float:
        ...
        try:
            # 有描述的实体数量
            entities_with_desc = session.scalar(
                select(func.count())
            ...
            # 有意义描述的实体（长度>10字符）
            entities_with_meaningful_desc = session.scalar(
                select(func.count())
                .select_from(EntityMetadata)
    ...
    def _analyze_relationship_coverage(self, session, total_entities: int) -> float:
        ...
        try:
            # 有关系的实体数量
            entities_with_relations = session.scalar(
                select(func.count(func.distinct(EntityRelationship.source_entity)))
                .select_from(EntityRelationship)
            ) or 0
            
            # 多关系实体数量（>= 2个关系）
            entities_with_multiple_relations = session.execute(
                select(func.count())
                .select_from(
            ...
            # 关系质量评分（强度>0.5的关系比例）
            high_quality_relations = session.scalar(
                select(func.count())
                .select_from(EntityRelationship)
    ...
    def get_entity_quality_score(self, entity_id: str) -> Dict[str, Any]:
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                # 综合评分
                overall_score = (desc_score * 0.4 + relation_score * 0.4 + activity_score * 0.2)
