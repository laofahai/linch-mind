...
from models.database_models import EntityMetadata, EntityRelationship, UserBehavior, AIConversation
...
class DataCleanupEngine:
    ...
    def _find_stale_entities(self, session, limit: int) -> List[DataCleanupSuggestion]:
        ...
        try:
            ...
            stale_entities = session.execute(
            ...
            for entity in stale_entities:
                candidates.append(DataCleanupSuggestion(
                    entity_id=entity.id,
                    reason="长期未访问且使用频率低",
    ...
    def _find_isolated_entities(self, session, limit: int) -> List[DataCleanupSuggestion]:
        """查找无关联的孤立实体"""
        candidates = []
        
        try:
            isolated_entities = session.execute(
                select(EntityMetadata)
                .where(
                    and_(
                        ~EntityMetadata.id.in_(
                            select(EntityRelationship.source_entity)
                            .union(select(EntityRelationship.target_entity))
                        ),
    ...
    def _find_duplicate_entities(self, session, limit: int) -> List[DataCleanupSuggestion]:
        """查找重复或相似的实体"""
        candidates = []
        ...
        try:
            ...
            for name, count in duplicate_names:
                duplicate_entities = session.execute(
                    select(EntityMetadata)
    ...
    def cleanup_expired_data(self, days_threshold: int = 1095) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db_service.get_session() as session:
                ...
                cleanup_results["deleted_conversations"] = conversation_delete_result.rowcount
                ...
                cleanup_results["deleted_relationships"] = relationship_cleanup_result.rowcount
    ...
    def get_cleanup_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                isolated_entities = session.scalar(
                    select(func.count())
                    .select_from(EntityMetadata)
                    .where(
                        ~EntityMetadata.id.in_(
                ...
                # 孤儿关系统计
                orphaned_relationships = session.scalar(
                    select(func.count())
                ...
                return {
                    ...
                    "cleanup_potential_mb": round((stale_entities + isolated_entities) * 0.001, 2)  # 估算
                }
