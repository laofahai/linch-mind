    ...
    def apply_migration(self, migration: Dict[str, str]) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(f"✅ 迁移 {migration['version']} 应用成功 ({execution_time}ms)")
                return True
...
"""
    ...
    def get_migration_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            "pending_migrations": [f"{m['version']}_{m['name']}" for m in pending_migrations],
...
if __name__ == "__main__":
    ...
    if command == "status":
        ...
        print(f"Applied: {status['applied_count']}, Pending: {status['pending_count']}, Failed: {status['failed_count']}")
