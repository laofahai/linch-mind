...
class UnixSocketStrategy(IPCStrategy):
    ...
    def get_connection_info(self) -> Dict[str, Any]:
        ...
        return {
            "type": "unix_socket",
            "socket_path": self.socket_path or (self._server.socket_path if self._server else None),
            "platform": "unix"
        }
...
class WindowsNamedPipeStrategy(IPCStrategy):
    ...
    async def start(self, app, security_manager, **kwargs) -> None:
        ...
        try:
            ...
            logger.info(f"Windows Named Pipe IPC服务器已启动: {self._server.full_pipe_name}")
