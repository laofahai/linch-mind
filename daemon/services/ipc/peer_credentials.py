...
class SocketPeerResolver:
    ...
    def _get_fallback_credentials(self, sock: socket.socket) -> PeerCredentials:
        ...
        try:
            ...
            logger.debug(f"回退方法：扫描进程连接，peer={peer_info}, local={local_info}")
            ...
            for proc in psutil.process_iter(['pid', 'connections']):
                try:
                    ...
                    for conn in connections:
                        if (conn.family == socket.AF_UNIX and
                            hasattr(conn, 'laddr') and conn.laddr):
    ...
    def validate_credentials(self, credentials: PeerCredentials) -> bool:
        ...
        try:
            ...
            # 没有psutil时，进行基本检查
            if credentials.confidence == "high" and credentials.source in ["SO_PEERCRED", "LOCAL_PEERPID"]:
                return True
            return False
        except Exception as e:
