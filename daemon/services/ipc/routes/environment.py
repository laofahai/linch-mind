...
from services.ipc.core.protocol import IPCErrorCode, IPCRequest, IPCResponse
...
class EnvironmentRoutes:
    ...
    def _ensure_services(self):
        ...
        if self.env_manager is None:
            ...
            self.env_manager = get_service(EnvironmentManager)
...
def create_environment_router():
    """创建环境管理路由器"""
    from services.ipc.core.router import IPCRouter

    router = IPCRouter(prefix="/environment")

    # 注册环境管理路由 - 使用直接注册方式
    from ..core.router import RoutePattern
