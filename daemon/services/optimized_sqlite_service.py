    ...
    def _create_reality_based_indexes(self, conn):
        ...
        for index_sql in indexes:
            try:
                ...
                logger.debug(f"创建索引成功: {index_sql.split('idx_')[1].split(' ')[0]}")
            except Exception as e:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                return {
                    ...
                    "cache_size_mb": round(abs(cache_size) / 256, 2) if cache_size < 0 else "N/A",
                    "tables": tables,
            ...
            with self.engine.connect() as conn:
                ...
                for table in tables:
                    try:
                        ...
                        # 表大小估算（页数）
                        pages_result = conn.execute(text(f"SELECT COUNT(*) FROM pragma_table_info('{table}')"))
