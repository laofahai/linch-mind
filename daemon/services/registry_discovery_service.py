...
class RegistryDiscoveryService:
    ...
    async def discover_registry(self, force_refresh: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[RegistrySource]]:
        """
        发现并获取注册表
        返回: (注册表数据, 成功的源)
        """
        ...
        for source in self.sources:
            ...
            try:
                ...
                # 记录失败次数
                self._failure_count[source.type] = self._failure_count.get(source.type, 0) + 1
                logger.warning(f"从 {source.description} 获取注册表失败: {e}")
                
                # 如果某个源连续失败多次，临时禁用
                if self._failure_count[source.type] >= 3:
                    logger.warning(f"源 {source.description} 连续失败 {self._failure_count[source.type]} 次，临时禁用")
                    source.enabled = False
        
        logger.error("所有注册表源都失败，无法获取注册表数据")
        return None, None
    
    async def _fetch_from_source(self, source: RegistrySource) -> Optional[Dict[str, Any]]:
        """从指定源获取注册表数据"""
    ...
    async def _save_to_cache(self, registry_data: Dict[str, Any], source: RegistrySource):
        """保存到本地缓存"""
        try:
            # 保存注册表数据
            ...
            meta = {
                ...
                'data_hash': hashlib.md5(json.dumps(registry_data, sort_keys=True).encode()).hexdigest(),
    ...
    def get_discovery_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            } if self._last_successful_source else None,
            "cache_info": {
