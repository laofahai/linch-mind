...
class SelectiveEncryptedStorage:
    ...
    def _analyze_graph_sensitivity(self, graph_data: nx.MultiDiGraph) -> Dict[str, Any]:
        ...
        try:
            ...
            # 规则1: 大型图（节点数 > 100）
            if analysis['node_count'] > self.sensitivity_thresholds['large_graph_node_count']:
                reasons.append(f"大型图数据 ({analysis['node_count']} 节点)")
            
            # 规则2: 包含敏感边类型
            if analysis['sensitive_edges'] > 0:
                sensitive_ratio = analysis['sensitive_edges'] / max(analysis['edge_count'], 1)
                if sensitive_ratio > self.sensitivity_thresholds['sensitive_ratio_threshold']:
            ...
            if analysis['sensitive_nodes'] > 0:
                sensitive_node_ratio = analysis['sensitive_nodes'] / max(analysis['node_count'], 1)
    ...
    def _save_encrypted(self, graph_data: nx.MultiDiGraph, filepath: Path, analysis: Dict[str, Any]) -> bool:
        """保存加密图数据"""
        try:
            # 序列化图数据
            ...
            logger.info(f"图数据已加密保存: {encrypted_file} ({len(encrypted_data)} bytes)")
    ...
    def _save_plaintext(self, graph_data: nx.MultiDiGraph, filepath: Path, analysis: Dict[str, Any]) -> bool:
        """保存明文图数据"""
        try:
            # 序列化并保存
...
def test_selective_encryption():
    ...
    try:
        ...
        for i in range(150):  # 超过阈值的节点数
            sensitive_graph.add_node(f"person_{i}", entity_type="person", name=f"联系人{i}")
        
        # 添加敏感关系
        for i in range(20):
            sensitive_graph.add_edge(f"person_{i}", f"person_{i+1}", 
        ...
        if success2:
            ...
            print(f"节点数量: {loaded_graph2.number_of_nodes() if loaded_graph2 else 0}")
