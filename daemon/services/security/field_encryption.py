...
class EncryptedString(TypeDecorator):
    ...
    def process_bind_param(self, value: Any, dialect) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                f"字段加密: {len(value)} bytes -> {len(encrypted_value)} bytes"
            )
    ...
    def process_result_value(self, value: Any, dialect) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                f"字段解密: {len(value)} bytes -> {len(decrypted_value)} bytes"
            )
...
class EncryptedText(TypeDecorator):
    ...
    def process_bind_param(self, value: Any, dialect) -> Optional[str]:
        ...
        try:
            ...
            encrypted_value = manager.encrypt_field(value)
            logger.debug(
                f"长文本字段加密: {len(value)} bytes -> {len(encrypted_value)} bytes"
            )
            return encrypted_value
        except Exception as e:
    ...
    def process_result_value(self, value: Any, dialect) -> Optional[str]:
        ...
        try:
            ...
            decrypted_value = manager.decrypt_field(str(value))
            logger.debug(
                f"长文本字段解密: {len(value)} bytes -> {len(decrypted_value)} bytes"
            )
            return decrypted_value
        except Exception as e:
