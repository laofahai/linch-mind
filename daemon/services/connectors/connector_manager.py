...
class ConnectorManager:
    ...
    def __init__(self, connectors_dir=None, db_service=None, process_manager=None, config_service=None, registry_service=None, config_manager=None):
        ...
        self.registry_service = registry_service or get_service(ConnectorRegistryService)
    ...
    async def register_connector(
        ...
        try:
            ...
            with self.db_service.get_session() as session:
                # 检查是否已存在
                existing = session.query(Connector).filter_by(
                    connector_id=connector_id
                ).first()
                ...
                if existing:
                    ...
                    if "entry" in connector_config:
                        existing.path = self._resolve_connector_executable_path(connector_id, connector_config)
                    
                    logger.info(f"更新连接器: {connector_id}")
                else:
                    # 创建新连接器并设置可执行文件路径
                    executable_path = self._resolve_connector_executable_path(connector_id, connector_config)
                    ...
                    logger.info(f"注册新连接器: {connector_id}, 可执行文件: {executable_path}")
    ...
    def scan_directory_for_connectors(self, connectors_dir: str) -> List[Dict[str, Any]]:
    ...
    def register_connector_from_path(self, connector_path: str) -> bool:
        ...
        try:
            ...
            # 注册连接器
            asyncio.run(self.register_connector(
                connector_id=connector_id,
                name=name,
                path=str(path),
    ...
    async def start_all_registered_connectors(self) -> None:
        ...
        try:
            with self.db_service.get_session() as session:
                # 获取所有启用的连接器
                enabled_connectors = session.query(Connector).filter_by(
                    enabled=True
                ).all()
            ...
            for connector in enabled_connectors:
                if connector.status != "running":
                    logger.info(f"启动连接器: {connector.name} ({connector.connector_id})")
                    ...
                    # 检查实际进程状态
                    actual_pid = self.process_manager.get_running_pid(connector.connector_id)
                    if actual_pid:
                        # 进程确实存在，同步到ProcessManager内存
                        if connector.connector_id not in self.process_manager.running_processes:
                            try:
                                # 同步现有进程到内存状态
                                self.process_manager.running_processes[connector.connector_id] = {
                                    "pid": actual_pid,
                                ...
                                logger.info(f"连接器已在运行，已同步到内存: {connector.name} (PID: {actual_pid})")
                            except Exception as e:
                                logger.error(f"同步连接器状态到内存失败 {connector.connector_id}: {e}")
                        else:
                            logger.info(f"连接器已在运行: {connector.name} (PID: {actual_pid})")
                    else:
                        # 数据库显示运行但进程不存在，修正数据库状态
                        logger.warning(f"连接器 {connector.name} 数据库显示运行但进程不存在，修正状态")
                        with self.db_service.get_session() as session:
                            connector_in_db = session.query(Connector).filter_by(
                                connector_id=connector.connector_id
                            ).first()
                            if connector_in_db:
                                ...
                                session.commit()
                                logger.info(f"已修正连接器 {connector.name} 数据库状态为 stopped")
    ...
    async def stop_all_connectors(self) -> None:
        ...
        try:
            with self.db_service.get_session() as session:
                # 获取所有运行中的连接器
                running_connectors = session.query(Connector).filter_by(
                    status="running"
                ).all()
    ...
    def _resolve_connector_executable_path(self, connector_id: str, connector_config: Dict[str, Any]) -> Optional[str]:
        """解析连接器的可执行文件路径
    ...
    def get_process_info(self, connector_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            with self.db_service.get_session() as session:
                ...
                if process_status.get("status") == "running" and connector.status != "running":
                    logger.info(f"同步连接器状态: {connector_id} 数据库状态更新为 running")
                    connector.status = "running"
                ...
                elif process_status.get("status") in ["not_running", "dead"] and connector.status == "running":
                    logger.info(f"同步连接器状态: {connector_id} 数据库状态更新为 stopped")
                    connector.status = "stopped"
                    connector.process_id = None
                    connector.updated_at = datetime.now(timezone.utc)
                    session.commit()
