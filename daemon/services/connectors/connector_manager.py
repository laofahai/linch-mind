...
class ConnectorManager:
        ...
        try:
            ...
            if config and "path" in config:
                ...
                if (
                    ...
                    and (provided_path / "connector.json").exists()
                ):
    ...
    async def start_connector(self, connector_id: str) -> bool:
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                # 从配置中获取可执行路径
                if not connector.config_data or not connector.config_data.get(
                    "executable_path"
                ):
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                if connector.status == "running":
                    if not force:
                        logger.error(
                            f"连接器正在运行，请先停止或使用强制删除: {connector_id}"
                        )
                        return False
                    else:
                ...
                # 先删除相关的配置历史记录
                from daemon.models.database_models import (ConnectorConfigHistory,
                                                           ConnectorLog, ConnectorStats)

                # 删除配置历史
                session.query(ConnectorConfigHistory).filter_by(
                    connector_id=connector_id
                ).delete()

                # 删除日志记录
                session.query(ConnectorLog).filter_by(
                    connector_id=connector_id
                ).delete()

                # 删除统计记录
                session.query(ConnectorStats).filter_by(
                    connector_id=connector_id
                ).delete()
    ...
    def _check_process_status(
        self, connector_id: str, process_id: Optional[int]
    ) -> Dict[str, Any]:
        """检查进程实际状态"""
        if not process_id:
            return {
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                for c in connectors:
                    ...
                    if actual_status != c.status and actual_status in [
                        "stopped",
                        "error",
                    ]:
    ...
    async def start_all_registered_connectors(self):
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                logger.info(
                    f"自动启动策略: 启动了 {len(enabled_connectors)} 个启用的连接器"
                )
        except Exception as e:
            ...
            def _scan_recursive(path: Path, max_depth: int = 3, current_depth: int = 0):
                ...
                if config_file.exists():
                    try:
                        ...
                        category = (
                            config.get("category", "local_files") or "local_files"
                        )
                        ...
                        connector_info = {
                            ...
                            "config_default_values": config.get(
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    or connector.connector_id in process.cmdline()
                ):
                    # 检查心跳超时
                    if connector.last_heartbeat:
                        time_since_heartbeat = (
                            datetime.now(timezone.utc) - connector.last_heartbeat
                        )
    ...
    def get_detailed_connector_status(
    ...
    def _get_connector_statistics(self, connector_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"data_sources": 0, "entities": 0, "error": str(e)}
    ...
    async def auto_restart_failed_connectors(self) -> List[Dict[str, Any]]:
        ...
        try:
            with self.db_service.get_session() as session:
                failed_connectors = (
                    session.query(Connector).filter_by(status="error").all()
                )
                ...
                for connector in failed_connectors:
                    if connector.enabled:  # 只重启启用的连接器
                        try:
                            logger.info(
                                f"尝试自动重启失败的连接器: {connector.connector_id}"
                            )
                            ...
                            logger.error(
                                f"自动重启连接器失败 {connector.connector_id}: {e}"
                            )
                            restart_results.append(
                                {
                                    "connector_id": connector.connector_id,
    ...
    async def install_connector(
        self, connector_id: str, source: str, config: dict = None
    ) -> bool:
        """安装连接器（暂时简化实现）"""
        try:
