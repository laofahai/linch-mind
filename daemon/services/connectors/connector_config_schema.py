...
class ConnectorConfigSchema(BaseModel):
    ...
    def to_ui_schema(self) -> Dict[str, Any]:
        ...
        # 按section组织UI
        for section_id, section in sorted(self.sections.items(), key=lambda x: x[1].order):
            ...
            # 字段UI配置
            for field_name, field_schema in sorted(section.fields.items(), key=lambda x: x[1].order):
                field_ui = {
                    "ui:widget": field_schema.widget.value if field_schema.widget else None,
...
def create_basic_config_schema(connector_id: str, connector_name: str) -> ConnectorConfigSchema:
    """创建基础配置Schema模板"""
