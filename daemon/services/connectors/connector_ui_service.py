...
class ConnectorUIService:
    ...
    async def register_custom_widget(
        ...
        try:
            ...
            if "actions" in widget_config:
                await self._register_widget_actions(
                    connector_id, widget_name, widget_config["actions"]
                )
    ...
    async def get_custom_widget_config(
        ...
        try:
            ...
            if not widget_info:
                # 尝试从连接器代码动态加载
                widget_config = await self._load_widget_from_connector(
                    connector_id, widget_name
                )
                if widget_config:
                    await self.register_custom_widget(
                        connector_id, widget_name, widget_config
                    )
    ...
    async def get_iframe_content(
        self, connector_id: str, component_name: str, request_params: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """获取iframe组件的HTML内容"""
        try:
            ...
            # 尝试从连接器目录加载静态HTML文件
            static_content = await self._load_static_iframe_content(
                connector_id, component_name
            )
            if static_content:
                return static_content

            # 生成默认iframe内容
            return await self._generate_default_iframe_content(
                connector_id, component_name, request_params
            )
    ...
    async def get_static_file(
        self, connector_id: str, file_path: str
    ) -> Optional[Dict[str, Any]]:
        """获取连接器静态文件信息"""
        try:
            # 查找连接器目录
    ...
    async def validate_custom_config(
        self, connector_id: str, widget_name: str, config_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """验证自定义组件配置"""
        try:
            widget_config = await self.get_custom_widget_config(
                connector_id, widget_name
            )
            if not widget_config:
                return {"valid": False, "errors": [f"未找到组件 {widget_name} 的配置"]}
    ...
    async def get_available_widgets(self, connector_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 扫描连接器目录中的组件定义
            discovered_widgets = await self._discover_widgets_from_connector(
                connector_id
            )
    ...
    async def execute_widget_action(
        ...
        try:
            ...
            return {"success": False, "message": f"未找到动作处理器: {action_name}"}

        except Exception as e:
            logger.error(
                f"执行组件动作失败 {connector_id}:{widget_name}:{action_name}: {e}"
            )
    ...
    async def _register_iframe_handler(
        ...
        if "template_path" in config:
            ...
            async def template_handler(params):
                return await self._render_template_iframe(
                    connector_id, config["template_path"], params
                )
            ...
            async def static_handler(params):
                return await self._load_static_iframe_content(
                    connector_id, config["static_file"]
                )

            self._iframe_handlers[handler_key] = static_handler

    async def _register_widget_actions(
        self, connector_id: str, widget_name: str, actions: Dict[str, Any]
    ):
        """注册组件动作处理器"""
    ...
    async def _load_static_iframe_content(
        self, connector_id: str, file_name: str
    ) -> Optional[Dict[str, Any]]:
        """加载静态iframe内容"""
        try:
            connector_path = self._find_connector_path(connector_id)
    ...
    async def _generate_default_iframe_content(
        self, connector_id: str, component_name: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """生成默认iframe内容"""
        html_content = f"""
        <!DOCTYPE html>
    ...
    async def _discover_widgets_from_connector(
    ...
    async def _execute_action_config(
        self, connector_id: str, action_config: Dict[str, Any], params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行动作配置"""
        ...
        if action_type == "http":
            ...
            # 脚本执行动作
            return await self._execute_script_action(
                connector_id, action_config, params
            )
        else:
    ...
    async def _execute_script_action(
        self, connector_id: str, config: Dict[str, Any], params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行脚本动作"""
        # 简化实现
    ...
    async def _load_widget_from_python_code(
        self, connector_path: Path, widget_name: str
    ) -> Optional[Dict[str, Any]]:
        """从Python代码加载组件定义"""
    ...
    async def _render_template_iframe(
        self, connector_id: str, template_path: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """渲染模板iframe"""
        # 实际实现可以使用Jinja2等模板引擎
        return await self._generate_default_iframe_content(
            connector_id, template_path, params
        )
