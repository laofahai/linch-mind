...
from .connector_config_schema import ConnectorConfigSchema, create_basic_config_schema
...
class ConnectorConfigService:
            ...
            with self.db_service.get_session() as session:
                ...
                if connector.config_schema:
                    ...
                    schema_data = {
                        ...
                        "default_values": connector.config.get("default_values", {}) if connector.config else {},
    ...
    async def register_runtime_schema(self, connector_id: str, schema_data: Dict[str, Any]):
        ...
        try:
            ...
            # 同步更新数据库
            await self._update_schema_in_database(connector_id, self._runtime_schemas[connector_id])
    ...
    async def update_config(
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                connector.config_validation_errors = validation_result.get("errors") if not validation_result["valid"] else None
                ...
                # 检查是否支持热重载
                hot_reload_applied = await self._try_hot_reload_config(connector_id, new_config)
    ...
    async def get_config_history(
        ...
        try:
            with self.db_service.get_session() as session:
                # 查询历史记录
                query = session.query(ConnectorConfigHistory)\
            ...
            with self.db_service.get_session() as session:
                ...
                for connector in connectors:
                    schema_info = {
                        ...
                        "has_runtime_schema": connector.connector_id in self._runtime_schemas,
                        "config_version": connector.config_version or "1.0.0",
                        "last_updated": connector.updated_at.isoformat() if connector.updated_at else None
                    }
    ...
    async def _load_schema_from_connector(self, connector_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                f"{connector_id}_connector", 
                connector_path / "main.py"
            )
            ...
            for attr_name in dir(module):
                ...
                if (isinstance(attr, type) and 
                    hasattr(attr, 'get_config_schema') and 
                    attr.__name__ != 'BaseConnector'):
    ...
    def _generate_default_schema(self, connector_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "default_values": self._extract_defaults_from_schema(schema_obj.to_json_schema()),
            "version": schema_obj.schema_version
        }
    
    def _extract_defaults_from_schema(self, json_schema: Dict[str, Any]) -> Dict[str, Any]:
        """从JSON Schema提取默认值"""
        defaults = {}
    ...
    async def _update_schema_in_database(self, connector_id: str, schema_data: Dict[str, Any]):
        """更新数据库中的schema"""
        try:
            with self.db_service.get_session() as session:
                connector = session.query(Connector).filter_by(connector_id=connector_id).first()
    ...
    async def _try_hot_reload_config(self, connector_id: str, new_config: Dict[str, Any]) -> bool:
        """尝试对运行中的连接器进行热重载"""
        try:
            # 检查连接器是否支持热重载
            schema_data = await self.get_config_schema(connector_id)
