...
class CachedNetworkXService:
    ...
    async def load_optimized_graph(self) -> bool:
        ...
        try:
            ...
            logger.info(f"数据加载完成: {len(entities)}个实体, {len(relationships)}个关系")
    ...
    async def _load_relationships_in_batches(self, batch_size: int = 2000) -> List[Dict]:
        """分批加载关系数据"""
        relationships = []
    ...
    def _build_optimized_graph(self, entities: List[Dict], relationships: List[Dict]) -> nx.Graph:
        """构建优化的图结构"""
        graph = nx.Graph()
        ...
        logger.info(f"图构建完成: {graph.number_of_nodes()}节点, {graph.number_of_edges()}边")
        return graph
    ...
    def _build_graph_query_indexes(self):
        ...
        logger.info(f"查询索引构建完成: {len(self.type_index)}个实体类型, {len(self.edge_type_index)}个关系类型")
    ...
    async def _precompute_common_metrics(self):
        ...
        try:
            ...
            logger.info(f"图指标预计算完成 - 度中心性: {degree_time:.2f}s, PageRank: {pagerank_time:.2f}s")
            
        except Exception as e:
            logger.warning(f"图指标预计算失败: {e}")
            self.precomputed_metrics = {}

    async def cached_find_related(self, entity_id: str, max_depth: int = 2, min_strength: float = 0.1) -> List[Dict]:
        """带缓存的关系查找"""
        ...
        # 执行查询
        try:
            result = await self._optimized_bfs_search(entity_id, max_depth, min_strength)
    ...
    async def _optimized_bfs_search(self, entity_id: str, max_depth: int, min_strength: float) -> List[Dict]:
        """优化的BFS搜索算法"""
                    ...
                    if neighbor_id not in visited:
                        ...
                        if strength >= min_strength:
                            ...
                            related_entities.append({
                                ...
                                'relationship_type': edge_data.get('type', 'related') if edge_data else 'related',
                                'strength': strength,
                                'depth': depth + 1,
    ...
    async def get_entity_centrality(self, entity_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            if 'degree_centrality' in self.precomputed_metrics:
                centrality_metrics['degree'] = self.precomputed_metrics['degree_centrality'].get(entity_id, 0.0)
            
            # PageRank（来自预计算）
            if 'pagerank' in self.precomputed_metrics:
                centrality_metrics['pagerank'] = self.precomputed_metrics['pagerank'].get(entity_id, 0.0)
    ...
    async def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "load_time": self.last_load_time.isoformat() if self.last_load_time else None
            }
