            ...
            if not platform.system() == 'Windows':
                ...
                try:
                    ...
                    if context.client_uid == self.daemon_uid:
                        ...
                        logger.info(f"IPC进程身份验证通过: PID={client_pid}, UID={context.client_uid}")
                    else:
                        logger.warning(f"IPC进程身份验证失败: PID={client_pid}, UID={context.client_uid} != {self.daemon_uid}")
...
class IPCFirewall:
    ...
    def is_path_allowed(self, path: str, client_context: SecurityContext) -> bool:
        ...
        if path in self.blocked_paths:
            logger.warning(f"IPC访问被阻止的路径: {path} (PID={client_context.client_pid})")
            return False
        
        if path in self.sensitive_paths:
            # 敏感路径需要额外验证
            if not client_context.authenticated:
                logger.warning(f"IPC未认证访问敏感路径: {path} (PID={client_context.client_pid})")
                return False
