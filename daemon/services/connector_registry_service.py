...
class ConnectorRegistryService:
    ...
    def _get_registry_url(self) -> str:
        ...
        # 2. 配置文件
        try:
            config_data = self.config.get_raw_config()
    ...
    async def fetch_registry(self, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
        """获取连接器注册表"""
        try:
            # 检查缓存
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.registry_url, timeout=30) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"成功获取注册表，包含 {len(registry_data.get('connectors', {}))} 个连接器")
    ...
    async def get_available_connectors(self, force_refresh: bool = False) -> List[Dict[str, Any]]:
        """获取可用连接器列表"""
        try:
    ...
    async def get_connector_download_info(self, connector_id: str, platform: str = 'linux-x64') -> Optional[Dict[str, Any]]:
        """获取连接器下载信息"""
        try:
    ...
    async def search_connectors(self, query: str, category: Optional[str] = None) -> List[Dict[str, Any]]:
        """搜索连接器"""
        try:
