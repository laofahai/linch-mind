...
class DaemonDiscovery:
    ...
    def discover_daemon(self) -> Optional[DaemonInfo]:
        ...
        if daemon_info.is_accessible:
            logger.info(f"Discovered accessible daemon at {daemon_info.host}:{daemon_info.port}")
        else:
            logger.warning(f"Daemon found but not accessible at {daemon_info.host}:{daemon_info.port}")
    ...
    def _verify_daemon_process(self, pid: int) -> bool:
        ...
        try:
            ...
            if 'python' in proc.name().lower():
                ...
                if any(keyword in cmdline.lower() for keyword in ['api/main', 'daemon', 'linch-mind', 'uvicorn']):
                    return True
    ...
    def _test_daemon_connection(self, host: str, port: int, timeout: float = 3.0) -> bool:
        """测试daemon连接性"""
        try:
    ...
    def wait_for_daemon(self, timeout: float = 30.0, check_interval: float = 1.0) -> Optional[DaemonInfo]:
        """等待daemon启动
