...
class TestSocketPeerResolver(unittest.TestCase):
    ...
    @unittest.skipIf(platform.system() == "Windows", "Unix socket test not supported on Windows")
    ...
    def test_fallback_credentials_success(self, mock_process_iter):
        ...
        mock_proc.info = {
            'pid': current_pid + 1,
            'connections': [
                Mock(family=socket.AF_UNIX, laddr='/tmp/test.sock')
            ]
        }
    ...
    def test_real_unix_socket_connection(self):
        ...
        try:
            ...
            if server_peer_creds:
                print(f"获取的凭证: PID={server_peer_creds.pid}, 来源={server_peer_creds.source}, 可信度={server_peer_creds.confidence}")
                ...
                # 检查来源是否为平台特定的高可信度来源
                if platform.system() == "Linux":
                    self.assertIn(server_peer_creds.source, ["SO_PEERCRED", "psutil_scan", "fallback_failed"])
                elif platform.system() == "Darwin":
                    self.assertIn(server_peer_creds.source, ["LOCAL_PEERPID", "psutil_scan", "fallback_failed"])
            else:
