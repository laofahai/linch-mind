...
class TestAPIIntegrationEffective:
    ...
    def test_connector_lifecycle_api_endpoints(self, test_client):
        ...
        register_response = client.post("/api/v1/connectors/register", json=new_connector)
        if register_response.status_code in [200, 201]:
            ...
            if start_response.status_code in [200, 202]:
                ...
                # 7. 停止连接器
                stop_response = client.post("/api/v1/connectors/test_api_connector/stop")
            ...
            if unregister_response.status_code in [200, 204]:
                ...
                if final_list_response.status_code == 200:
                    ...
                    # 确认测试连接器不在列表中
                    test_connector = next(
                        (c for c in final_connectors if c["connector_id"] == "test_api_connector"),
                        None
                    )
    ...
    def test_api_error_handling(self, test_client):
        ...
        assert response.status_code in [400, 422]  # 400 Bad Request 或 422 Validation Error
    ...
    def test_api_data_validation(self, test_client):
        ...
        invalid_inputs = [
            ...
            # 无效的配置数据
            {"connector_id": "test", "name": "Test", "config": "invalid_config_not_dict"},
        ]
        
        for invalid_input in invalid_inputs:
            response = client.post("/api/v1/connectors/register", json=invalid_input)
            # 应该返回错误状态码
            assert response.status_code in [400, 422], f"Invalid input should be rejected: {invalid_input}"
    ...
    def test_api_concurrent_requests(self, test_client):
        ...
        def make_concurrent_request(index):
            ...
            results.append((index, response.status_code, response.json() if response.status_code < 400 else None))
...
class TestAPIEdgeCases:
    ...
    def test_special_characters_in_requests(self, test_client):
        ...
        special_cases = [
            ...
            # HTML/XML字符
            {"connector_id": "html_connector", "name": "<script>alert('test')</script>"},
        ...
        for test_case in special_cases:
            ...
            if response.status_code in [200, 201]:
                # 如果接受了，验证数据被正确存储和转义
                try:
                    info_response = client.get(f"/api/v1/connectors/{test_case['connector_id']}")
                    if info_response.status_code == 200:
                        connector_info = info_response.json()
    ...
    def test_missing_content_type_header(self, test_client):
        ...
        assert response.status_code in [200, 201, 400, 415]  # 415 = Unsupported Media Type