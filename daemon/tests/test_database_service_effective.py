...
from models.database_models import Entity, Relationship, UserBehavior, RecommendationFeedback, Base
...
class TestDatabaseServiceEffective:
    ...
    async def test_database_initialization_and_schema(self, real_db_service):
        ...
        async with db_service.get_session() as session:
            ...
            # 验证必要的表存在
            expected_tables = ['entities', 'relationships', 'user_behaviors', 'recommendation_feedbacks']
    ...
    async def test_entity_crud_operations(self, real_db_service):
        ...
        update_data = {
            ...
            "metadata": {"category": "technology", "difficulty": "beginner", "popularity": "high"}
        }
    ...
    async def test_relationship_operations(self, real_db_service):
        ...
        # 应该能找到指向该实体的关系
        incoming_relations = [r for r in reverse_relationships if r["source_entity_id"] == entity1_id]
    ...
    async def test_search_functionality(self, real_db_service):
        ...
        # 创建多个实体用于搜索
        entities_to_create = [
            {"name": "Python Programming", "entity_type": "skill", "content": "Programming with Python"},
            {"name": "Java Development", "entity_type": "skill", "content": "Enterprise Java development"},
            {"name": "Python Libraries", "entity_type": "resource", "content": "Useful Python libraries"},
            {"name": "Machine Learning", "entity_type": "field", "content": "Artificial intelligence field"}
        ]
    ...
    async def test_user_behavior_tracking(self, real_db_service):
        ...
        # 验证行为数据
        view_behavior = next((b for b in user_behaviors if b["action"] == "view_entity"), None)
        assert view_behavior is not None
        assert view_behavior["entity_id"] == entity_id
        assert view_behavior["metadata"]["duration"] == 120
        
        search_behavior = next((b for b in user_behaviors if b["action"] == "search"), None)
    ...
    async def test_recommendation_feedback(self, real_db_service):
        ...
        # 查询用户的反馈
        user_feedbacks = await db_service.get_user_recommendation_feedbacks("test_user_456")
        assert len(user_feedbacks) == 2
        
        # 验证反馈数据
        positive_feedback = next((f for f in user_feedbacks if f["feedback_type"] == "positive"), None)
        ...
        negative_feedback_retrieved = next((f for f in user_feedbacks if f["feedback_type"] == "negative"), None) 
    ...
    async def test_concurrent_database_operations(self, real_db_service):
        ...
        concurrent_entities = [e for e in all_entities if e["entity_type"] == "concurrent_test"]
    ...
    async def test_database_constraints_and_validation(self, real_db_service):
        ...
        try:
            ...
            entities = await db_service.search_entities("")
            # 根据实现决定是否应该找到这个实体
    ...
    async def test_transaction_rollback_on_error(self, real_db_service):
        ...
        try:
            async with db_service.get_session() as session:
                ...
                # 故意触发错误（例如，违反约束）
                await session.execute("INSERT INTO entities (id) VALUES (NULL)")  # 无效SQL
    ...
    async def test_database_performance_with_large_dataset(self, real_db_service):
        ...
        performance_entities = [e for e in all_entities if e["entity_type"] == "performance_test"]
