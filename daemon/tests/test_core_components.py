...
class TestDatabaseManager:
    ...
    async def test_database_manager_initialization(self):
        ...
        config = DatabaseConfig(
            database_url="sqlite:///:memory:", pool_size=5, max_overflow=10
        )
    ...
    def test_session_context_manager(self):
        ...
        config = DatabaseConfig(
            database_url="sqlite:///:memory:", pool_size=2, max_overflow=5
        )
    ...
    def test_transaction_rollback_on_error(self):
        ...
        config = DatabaseConfig(
            database_url="sqlite:///:memory:", pool_size=1, max_overflow=1
        )
...
class TestIntegration:
    ...
    def test_exception_handler_with_error_codes(self):
        ...
        # 测试错误响应创建
        response = create_error_response(
            ErrorCode.DATABASE_CONNECTION_FAILED, details="集成测试错误"
        )
        assert response["error"]["code"] == ErrorCode.DATABASE_CONNECTION_FAILED
