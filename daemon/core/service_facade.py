...
from .container import get_container, ServiceNotRegisteredError
...
class ServiceFacade:
    ...
    def container(self):
        ...
        if self._container is None:
            ...
            logger.debug(f"ServiceFacade获取到容器，已注册服务数: {len(self._container.get_all_services())}")
    ...
    def get_service(self, service_type: Type[T], safe: bool = False) -> T:
        ...
        try:
            ...
            logger.debug(f"尝试获取服务 {service_name}，容器中已注册: {registered_services}")
