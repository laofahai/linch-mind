...
class DatabaseManager:
    ...
    def _initialize_engines(self):
        ...
        try:
            ...
            if self.config.enable_async and self.config.async_database_url:
                ...
                logger.info(
                    f"异步数据库引擎初始化完成: {self.config.async_database_url}"
                )
    ...
    async def get_async_session(self) -> AsyncSession:
        ...
        if not self._async_session_factory:
            raise DatabaseError(
                "异步数据库会话工厂未初始化", "ASYNC_SESSION_FACTORY_ERROR"
            )
        ...
        async with self._async_session_factory() as session:
            try:
                ...
                raise DatabaseError(
                    f"异步数据库操作失败: {e}", "ASYNC_TRANSACTION_ERROR"
                )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"healthy": False, "error": str(e), "stats": self.get_stats()}
...
def get_database_manager() -> DatabaseManager:
    ...
    if not _global_database_manager:
        raise DatabaseError(
            "数据库管理器未初始化，请先调用 initialize_database_manager()",
...
if __name__ == "__main__":
    ...
    # 创建测试配置
    config = DatabaseConfig(database_url="sqlite:///test.db", pool_size=5, echo=False)
