...
ERROR_CODE_MAP: Dict[ErrorCode, ErrorCodeInfo] = {
    ...
    ErrorCode.SUCCESS: ErrorCodeInfo(
        ErrorCode.SUCCESS, "Success", "操作成功完成", 
        ErrorCategory.GENERAL, ErrorSeverity.INFO
    ),
    ...
    ErrorCode.INVALID_REQUEST: ErrorCodeInfo(
        ErrorCode.INVALID_REQUEST, "Invalid Request", "无效的请求格式或参数",
        ErrorCategory.GENERAL, ErrorSeverity.WARNING,
        user_friendly_message="请求格式不正确"
    ),
    
    ErrorCode.RESOURCE_NOT_FOUND: ErrorCodeInfo(
        ErrorCode.RESOURCE_NOT_FOUND, "Resource Not Found", "请求的资源不存在",
        ErrorCategory.GENERAL, ErrorSeverity.WARNING,
    ...
    ErrorCode.IPC_CONNECTION_FAILED: ErrorCodeInfo(
        ErrorCode.IPC_CONNECTION_FAILED, "IPC Connection Failed", "IPC连接建立失败",
        ErrorCategory.IPC, ErrorSeverity.ERROR,
        user_friendly_message="无法连接到服务，请检查服务状态"
    ),
    
    ErrorCode.IPC_CONNECTION_LOST: ErrorCodeInfo(
        ErrorCode.IPC_CONNECTION_LOST, "IPC Connection Lost", "IPC连接意外断开",
        ErrorCategory.IPC, ErrorSeverity.ERROR,
        user_friendly_message="与服务的连接已断开，正在尝试重新连接"
    ),
    
    ErrorCode.IPC_TIMEOUT: ErrorCodeInfo(
        ErrorCode.IPC_TIMEOUT, "IPC Timeout", "IPC操作超时",
        ErrorCategory.IPC, ErrorSeverity.WARNING,
    ...
    ErrorCode.CONNECTOR_NOT_FOUND: ErrorCodeInfo(
        ErrorCode.CONNECTOR_NOT_FOUND, "Connector Not Found", "指定的连接器不存在",
        ErrorCategory.CONNECTOR, ErrorSeverity.WARNING,
        user_friendly_message="未找到指定的连接器"
    ),
    
    ErrorCode.CONNECTOR_STARTUP_FAILED: ErrorCodeInfo(
        ErrorCode.CONNECTOR_STARTUP_FAILED, "Connector Startup Failed", "连接器启动失败",
        ErrorCategory.CONNECTOR, ErrorSeverity.ERROR,
        user_friendly_message="连接器启动失败，请检查配置"
    ),
    
    # 服务错误
    ErrorCode.SERVICE_NOT_AVAILABLE: ErrorCodeInfo(
        ErrorCode.SERVICE_NOT_AVAILABLE, "Service Not Available", "服务不可用",
        ErrorCategory.SERVICE, ErrorSeverity.ERROR,
        user_friendly_message="服务暂时不可用，请稍后重试"
    ),
    
    ErrorCode.SERVICE_DEPENDENCY_MISSING: ErrorCodeInfo(
        ErrorCode.SERVICE_DEPENDENCY_MISSING, "Service Dependency Missing", "服务依赖缺失",
