                ...
                for param_name, param in sig.parameters.items():
                    ...
                    if param.annotation != inspect.Parameter.empty:
                        if param.annotation in self._services:
                            ...
                            logger.warning(f"无法解析参数 {param_name} 的依赖: {param.annotation}")
...
def injectable(service_type: Type[T] = None, lifecycle: str = ServiceLifecycle.SINGLETON):
    """可注入服务装饰器"""
    def decorator(cls):
...
# 便捷注册函数
def register_singleton(service_type: Type[T], factory: Callable[[], T] = None) -> ServiceContainer:
    """便捷单例注册函数"""
    return _global_container.register_singleton(service_type, factory)


def register_transient(service_type: Type[T], factory: Callable[[], T] = None) -> ServiceContainer:
    """便捷瞬态注册函数"""
    return _global_container.register_transient(service_type, factory)


def register_scoped(service_type: Type[T], factory: Callable[[], T] = None) -> ServiceContainer:
    """便捷作用域注册函数"""
