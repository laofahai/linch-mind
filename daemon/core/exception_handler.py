...
class ExceptionClassifier:
    ...
    @classmethod
    def classify_exception(cls, exc: Exception, context: str = "") -> LinchMindException:
        """将通用异常分类为具体的业务异常"""
        message = str(exc).lower()
        context = context.lower()
        
        # IPC相关异常
        if any(keyword in message or keyword in context for keyword in cls.IPC_KEYWORDS):
            return IPCConnectionError(str(exc), "IPC_ERROR", {"original_type": type(exc).__name__})
        
        # 数据库相关异常
        if any(keyword in message or keyword in context for keyword in cls.DB_KEYWORDS):
            return DatabaseError(str(exc), "DATABASE_ERROR", {"original_type": type(exc).__name__})
        
        # 连接器相关异常
        if any(keyword in message or keyword in context for keyword in cls.CONNECTOR_KEYWORDS):
            return ConnectorError(str(exc), "CONNECTOR_ERROR", {"original_type": type(exc).__name__})
        
        # 配置相关异常
        if any(keyword in message or keyword in context for keyword in cls.CONFIG_KEYWORDS):
            return ConfigurationError(str(exc), "CONFIG_ERROR", {"original_type": type(exc).__name__})
        
        # 默认为通用LinchMind异常
        return LinchMindException(str(exc), "UNKNOWN_ERROR", {"original_type": type(exc).__name__})
...
class StructuredExceptionHandler:
    ...
    def handle_with_logging(self, operation: str, reraise: bool = True, classify: bool = True):
        """装饰器：为函数添加结构化异常处理
            ...
            async def async_wrapper(*args, **kwargs):
                try:
                    ...
                    if classify:
                        ...
                        structured_exc = LinchMindException(str(e), "OPERATION_ERROR", {
                            "operation": operation,
    ...
    def _log_exception(self, exc: LinchMindException, operation: str, exc_info: bool = False):
        """记录结构化异常"""
        ...
        if isinstance(exc, (ServiceUnavailableError, ConfigurationError)):
            self.logger.critical(f"严重错误 in {operation}: {exc.message}", extra=log_data, exc_info=exc_info)
        elif isinstance(exc, (DatabaseError, IPCConnectionError)):
            self.logger.error(f"操作失败 in {operation}: {exc.message}", extra=log_data, exc_info=exc_info)
        elif isinstance(exc, (ValidationError, ConnectorError)):
            self.logger.warning(f"业务异常 in {operation}: {exc.message}", extra=log_data)
        else:
            self.logger.error(f"未知错误 in {operation}: {exc.message}", extra=log_data, exc_info=exc_info)
...
def get_exception_handler(logger: Optional[logging.Logger] = None) -> StructuredExceptionHandler:
    """获取异常处理器实例
...
# 便捷装饰器函数
def handle_exceptions(operation: str, reraise: bool = True, classify: bool = True, logger: logging.Logger = None):
    """便捷的异常处理装饰器"""
    handler = get_exception_handler(logger)
    return handler.handle_with_logging(operation, reraise, classify)


def safe_execute(func: Callable[..., T], *args, default: T = None, logger: logging.Logger = None, **kwargs) -> T:
    """安全执行函数，捕获所有异常并返回默认值"""
    handler = get_exception_handler(logger)
...
async def safe_execute_async(func: Callable[..., T], *args, default: T = None, logger: logging.Logger = None, **kwargs) -> T:
...
# 异常转换辅助函数
def ensure_structured_exception(exc: Exception, context: str = "") -> LinchMindException:
    """确保异常是结构化异常"""
