...
class DaemonPortService {
  ...
  Future<DaemonInfo?> discoverDaemon() async {
    ...
    try {
      ...
      if (!Platform.isWindows) {
        ...
        if ((mode & 0x3F) != 0) { // 检查group和other权限
      ...
      try {
        ...
        if (isAccessible) {
          ...
          print('[DaemonPortService] 发现可访问的daemon: ${daemonInfo.host}:${daemonInfo.port}');
        } else {
          print('[DaemonPortService] Daemon不可访问: ${daemonInfo.host}:${daemonInfo.port}');
        }
  ...
  /// 测试daemon连接
  Future<bool> _testDaemonConnection(DaemonInfo daemonInfo, {Duration timeout = const Duration(seconds: 3)}) async {
    try {
