...
class DaemonLifecycleService {
  ...
  static DaemonLifecycleService get instance =>
      _instance ??= DaemonLifecycleService._();
  ...
  Future<DaemonStartResult> _startDevelopmentDaemon() async {
    ...
    try {
      ...
      // 等待daemon启动
      final daemonInfo =
          await _portService.waitForDaemon(timeout: _startupTimeout);
      if (daemonInfo == null) {
        process.kill();
  ...
  Future<DaemonStartResult> _startProductionDaemon() async {
    ...
    try {
      ...
      return DaemonStartResult.failure('生产模式下daemon未运行。请确保Linch Mind系统服务已启动。');
    } catch (e) {
  ...
  Future<DaemonStartResult?> _checkSystemdService() async {
    try {
      final result =
          await Process.run('systemctl', ['is-active', 'linch-mind-daemon']);
  ...
  Future<DaemonStartResult?> _checkLaunchdService() async {
    try {
      final result = await Process.run(
          'launchctl', ['list', 'com.laofahai.linch-mind-daemon']);
