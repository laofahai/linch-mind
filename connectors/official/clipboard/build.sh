...
# Clipboard Connector Build Script
...
if [ ! -f "$SHARED_BUILD_SCRIPT" ]; then
    ...
    cd ..
    mkdir -p bin/release
    cp build/linch-mind-clipboard bin/release/
    echo "✅ Build completed: bin/release/linch-mind-clipboard"
else
    # Use shared build script
    exec "$SHARED_BUILD_SCRIPT" clipboard "$@"
fi