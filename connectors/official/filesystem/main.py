...
class FileSystemConnector(BaseConnector, FileSystemEventHandler):
                    ...
                    "properties": {
                        "default_extensions": {
                            ...
                            "predefined_options": [
                                ...
                                ".java", ".kt", ".go", ".rs", ".cpp", ".c", ".h", ".xml", ".sql", ".sh"
                            ]
                        },
                        "max_file_size": {
                            "type": "integer",
    ...
    async def _load_filesystem_config(self):
        ...
        self.supported_extensions = set(self.get_config('supported_extensions', 
            ['.txt', '.md', '.py', '.js', '.html', '.css', '.json', '.yaml', '.yml']))
        self.max_file_size = self.get_config('max_file_size', 1) * 1024 * 1024  # 转换为字节
        self.ignore_patterns = self.get_config('ignore_patterns', 
    ...
    async def _process_file_event(self, event_type: str, file_path: str):
        ...
        try:
            ...
            if success:
                self.logger.info(f"✅ 处理文件事件: {event_type} - {file_path_obj.name}")
            else:
