...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        if (arg == "--version" || arg == "-v") {
            std::cout << "linch-mind-system-info-connector 1.0.0" << std::endl;
            return 0;
        }
        
        // 帮助信息
        if (arg == "--help" || arg == "-h") {
            std::cout << "Linch Mind System Information Connector" << std::endl;
    ...
    std::cout << "🖥️ 启动 Linch Mind 系统信息连接器" << std::endl;
    
    try {
        // 创建系统信息连接器实例
        auto connector = std::make_unique<SystemInfoConnector>();
        ...
        std::cout << "✅ 系统信息连接器运行中，按 Ctrl+C 停止..." << std::endl;
        ...
        std::cout << "✅ 系统信息连接器已安全停止" << std::endl;
        return 0;
    }
    catch (const std::exception& e) {
        std::cerr << "❌ 系统信息连接器运行失败: " << e.what() << std::endl;
        return 1;
    }
}