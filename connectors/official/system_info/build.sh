...
# System Info Connector Build Script
...
if [ ! -f "$SHARED_BUILD_SCRIPT" ]; then
    ...
    cd ..
    mkdir -p bin/release
    cp build/linch-mind-system-info bin/release/
    echo "✅ Build completed: bin/release/linch-mind-system-info"
else
    # Use shared build script
    exec "$SHARED_BUILD_SCRIPT" system_info "$@"
fi