...
# User Context Connector Build Script
...
if [ ! -f "$SHARED_BUILD_SCRIPT" ]; then
    ...
    cd ..
    mkdir -p bin/release
    cp build/linch-mind-user-context bin/release/
    echo "✅ Build completed: bin/release/linch-mind-user-context"
else
    # Use shared build script
    exec "$SHARED_BUILD_SCRIPT" user_context "$@"
fi